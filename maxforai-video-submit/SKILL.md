...
| 用途 | 钥匙串 service | 该 Key 能打的模型（2026-08-17 实拉） |
|---|---|---|
| SD 2.5 | `seedance-workflow-maxforai-sd25` | `wf-sd2-5-720p` |
...
- 提交前必须 `GET /v1/models`（带对应 Key）核对：**Key 对不上模型会直接 401/无权限，不是"清单里没这个模型"**。
- 同一套文档里还有 `mg-seedance-2.5`、官方 `sd-*` 等，**本机这 3 把 Key 目前打不到**，别拿用户没开通的模型去试。
...
### 2.5 · `wf-sd2-5-720p`（Key: sd25）

- 计费文档价：**¥0.30/秒**，固定 720p，4–29 秒
- 最多 30 图 / 10 视频 / 10 音频；比例含 `9:16`
- 字段：`duration`(int) + `aspect_ratio` + `images`/`videos`/`audios`
...
10 秒 720p ≈ **¥3.00**。G01 类数字人优先这条（支持音频参考）。
