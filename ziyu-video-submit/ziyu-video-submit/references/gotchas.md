...
## 11. ⚠️ 音频参考总时长必须 ≤30 秒（2026-08-28 实测，早该知道）
- 参考音频超过 30 秒（例：005 声线 61s）→ **job 提交成功、不报错、照样扣费，但音频参考实际未生效**，成片对白是模型默认音色（不是参考声线）。这是"早知道的问题"：review-rules 明写「音频 10 段，总时长不超过 30 秒」。
- ✅ 提交前 `ffprobe` 检查音频时长；超过 30s 先 `ffmpeg -ss 15 -t 20 -i in.mp3 -codec:a libmp3lame -b:a 128k out.mp3` 裁到 ≤30s（取中段避开首尾静音）。
- 诊断：声音不对 → 先查音频时长 + 容器格式（mp3）+ mime，三者都过再怀疑提示词。

## 12. ⚠️ 宫格故事板作机位参考会被"占位人物"污染（2026-08-28 实测，待验证修复是否有效）
- 用甲方 storyboard 宫格图（多格分镜）作 @图片 锁机位时，**格内的占位/示意人物会被 2.5 读成真人形象**（B 镜出现戴眼镜中年男，不是 Johnny）。光靠提示词写「不采用人物」挡不住——图面信号比文字强。
- 处理（**待验证**）：用 PIL 把含人物的格做 `GaussianBlur(radius=10)` 强模糊，只保留构图几何（栏杆/海面/船型轮廓），重新上传。模糊后人物细节不可读，模型只能取机位。
- 配套：宫格读取顺序要写对（先确认布局是横排/竖排：`ImageStat` 看各 1/3 区 stddev，远景格 stddev 最高、空镜格最低）；提示词写「只采用机位和构图，不从 @图片N 任何一格采用人物的脸型/体型/发型/服装/肤色/眼镜等外形特征」。
- ⚠️ 验证点：v04 提交后看 B 镜人物是否回到 @图片1（001）的 Johnny 脸（不戴眼镜、无胡须差异）。

## 13. 成片下载续传（2026-08-28）
- 紫域 previewUrl 单次 curl 超时（30s 720p 成片 ~34MB，慢网 540s 只下 5MB）→ 用 Python `urllib` 流式分块写入（`read(1<<20)` 循环）后台下载，或 `curl -C -` 断点续传。
- 下载完成后 `ffprobe` 校验 duration/size，确认不是半成品（h264 "Invalid NAL unit size" 报错 = 文件不完整）。
